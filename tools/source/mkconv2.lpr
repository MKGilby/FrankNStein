...
// Some units are stripped because the file formats they support
// are not needed for this project. For full version of MKConv2
// see https://github.com/MKGilby/MKConv2
...
//  ARGBImageBMFReaderUnit, ARGBImageGIFReaderUnit,
  ...
  FileInfo,
  winpeimagereader, AnimXMLReader;
...
begin
  if not Help then begin
    ...
    if uppercase(Parms[1])='ANIMXML' then
      ...
      mems[area].Image.LoadFromFile(parms[2])
    else
      mems[area].Image.LoadFromFile(parms[2]);
    ...
    writeln('  to read animation data for an already read/created picture.');
    writeln('R <BMF|JSON> filename.ext area');
    writeln('  to read font data for an already read/created picture.');
    writeln;
...
begin
  if not Help then begin
    if Assigned(mems[strtoint(parms[3])]) then
      mems[strtoint(parms[3])].Image.SaveToFile(parms[2],parms[1]);
    writeln('File written.');
  end else begin
    writeln('Syntax:'#13#10);
    writeln('W [PNG|TGA|GSD|CEL|BMP|JSON] filename.ext area');
    ...
    writeln('  You can add animation data by reading a JSON or XML file ');
    writeln('  containing animation data over an already read picture.');
    writeln('  Written JSON file will contain only animation or font data.');
  end;
end;
...
begin
  ...
  CommandValidator.AddCommand('"W" <"PNG"|"TGA"|"GSD"|"CEL"|"BMP"|"JSON"> s i',Comm_W);
