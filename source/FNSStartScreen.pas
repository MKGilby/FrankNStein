...
begin
  ...
  repeat
    ...
    if (GetTickCount64-TickStart) mod (FLASHDELAY*2)<FLASHDELAY then
    ...
    MM.Fonts['Pink'].OutText('THIS REFURBICATION @2025 MKSZTSZ',LOGICALWINDOWWIDTH div 2,TEXTTOP+22,1);
    ...
    MM.Fonts['Purple'].OutText('AND LAZARUS 4.2 (FPC 3.2.2)',LOGICALWINDOWWIDTH div 2,TEXTTOP+66,1);

    {$ifndef LimitFPS} FlipNoLimit; {$else} Flip; {$endif}
    HandleMessages;
    if keys[SDL_SCANCODE_ESCAPE] or
       controllerbuttons[SDL_CONTROLLER_BUTTON_B] or
       Terminate then Result:=RES_TERMINATE;
    if keys[SDL_SCANCODE_RETURN] or
